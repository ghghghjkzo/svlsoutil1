...
CHARGES_M2_RE = re.compile(r"charges?\s*:?\s*(?:d'environ\s*)?(\d[\d\s]{0,4})\s?€\s?(?:HT)?\s?/\s?m²\s?/\s?an", re.I)
CHARGES_MENSUEL_RE = re.compile(r"(?:provisions?\s+pour\s+)?charges?\s*:?\s*(\d[\d\s]{0,6})\s?€?\s?(?:HT|TTC)?\s?(?:/|par)\s?mois", re.I)
CHARGES_ANNUEL_RE = re.compile(r"(?:provisions?\s+pour\s+)?charges?\s*:?\s*(\d[\d\s]{0,6})\s?€?\s?(?:HT|TTC)?\s?(?:/|par)\s?an", re.I)
...
def parse_listing_block(block: str, source_name: str, source_url: str,
    ...
    # rejette une valeur hors bornes plausibles : c'est une erreur d'extraction, pas une donnée
    if loyer_annuel_m2 is not None and not (LOYER_M2_AN_MIN <= loyer_annuel_m2 <= LOYER_M2_AN_MAX):
        loyer_annuel_m2 = None

    # charges : €/m²/an direct, sinon mensuel/annuel total ramené au m²
    charges_m2_an = None
    cm = CHARGES_M2_RE.search(block)
    if cm:
        charges_m2_an = _clean_int(cm.group(1))
    else:
        cmens = CHARGES_MENSUEL_RE.search(block)
        if cmens and surface:
            v = _clean_int(cmens.group(1))
            if v:
                charges_m2_an = round(v * 12 / surface, 1)
        else:
            cann = CHARGES_ANNUEL_RE.search(block)
            if cann and surface:
                v = _clean_int(cann.group(1))
                if v:
                    charges_m2_an = round(v / surface, 1)
    if charges_m2_an is not None and not (0 < charges_m2_an <= 150):
        charges_m2_an = None

    loyer_ht_hc_m2_an = (round(loyer_annuel_m2 + charges_m2_an, 1)
                         if (loyer_annuel_m2 is not None and charges_m2_an is not None) else None)
