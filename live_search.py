...
# Debug : texte brut des derniers blocs scrappés (rempli par _split_listing_blocks)
LAST_DEBUG_BLOCKS: list[str] = []
...
def _split_listing_blocks(soup: BeautifulSoup, base_url: str) -> list[tuple[str, str | None, str | None]]:
    ...
    # ── DEBUG : capture le texte brut des 3 premiers blocs pour diagnostic ──
    # Permet de voir EXACTEMENT ce que reçoivent les regex de prix quand
    # l'extraction échoue. Consultable via live_search.LAST_DEBUG_BLOCKS.
    global LAST_DEBUG_BLOCKS
    LAST_DEBUG_BLOCKS = [t[:600] for t, _, __ in kept[:3]]
...
def _fetch_one_photo(it: dict) -> tuple:
    ...
    Optimisé mémoire : libère le HTML et l'objet BeautifulSoup dès que possible
    (crucial sur les hébergements à faible RAM comme Render 512 Mo)."""
    updated = dict(it)
    soup = None
    try:
        ...
        if html:
            soup = BeautifulSoup(html, "html.parser")
            del html  # on n'a plus besoin du HTML brut → libéré immédiatement
        ...
        # décompose l'arbre BeautifulSoup pour libérer la RAM sans attendre le GC
        if soup is not None:
            try:
                soup.decompose()
            except Exception:
...
def fetch_photos_batch(items: list[dict], max_items: int = 40,
    ...
    parallel=True  (Rapide): traitement par petits lots de `workers` requêtes
                             simultanées. Rapide, mais plafonne la mémoire :
                             jamais plus de `workers` pages en RAM à la fois
                             (évite les crashs OOM sur Render 512 Mo).
    """
    ...
    if parallel and len(to_fetch) > 1:
        ...
        # ── Traitement par lots (chunks) plutôt que tout soumettre d'un coup ──
        # Soumettre les 40 tâches simultanément fait s'accumuler jusqu'à 40 pages
        # HTML + arbres BeautifulSoup en mémoire → pic RAM fatal sur 512 Mo.
        # En traitant par lots de `workers`, on borne le pic à `workers` pages.
        for start in range(0, len(to_fetch), workers):
            chunk = to_fetch[start:start + workers]
            with ThreadPoolExecutor(max_workers=workers) as ex:
                for key, updated in ex.map(_fetch_one_photo, chunk):
