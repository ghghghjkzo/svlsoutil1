...
# 1. Format explicite "167 € HT-HC/m²/an" (fiche BureauxLocaux) — le plus fiable
LOYER_HTHC_M2_AN_RE = re.compile(
    r"(\d[\d\s]{1,5})\s?€\s?HT-?HC\s?/\s?m²\s?/\s?an", re.I)
# 2. Format "180 € HT /m²/an" (sans tiret-HC)
LOYER_ANNUEL_M2_RE = re.compile(
    r"(\d[\d\s]{1,5})\s?€\s?(?:HT)?\s?/\s?m²\s?/\s?an", re.I)
# 3. Format mensuel explicitement HT-HC "4 500 € HT-HC/mois" → calculé en /m²/an
LOYER_HTHC_MENSUEL_RE = re.compile(
    r"(\d[\d\s]{1,7})\s?€\s?HT-?HC\s?/\s?mois", re.I)
...
def _split_listing_blocks(soup: BeautifulSoup, base_url: str) -> list[tuple[str, str | None]]:
    """Retourne une liste de (texte, lien_direct) — un par annonce détectée."""
    ...
    for c in candidates:
        ...
        if len(txt) > 60 and SURFACE_RE.search(txt):
            raw.append((txt, _extract_href(c, base_url, base_url)))

    if not raw:
        # repli : découpage du texte brut sur des marqueurs récurrents observés
        full_text = soup.get_text(" ", strip=True)
        parts = re.split(r"(?=Voir l'annonce|En savoir plus|Disponibilité\s?:)", full_text)
        raw = [(p, None) for p in parts if SURFACE_RE.search(p)]

    # élimine les doublons structurels : un conteneur parent qui englobe le texte
    # d'un conteneur enfant déjà capturé (texte strictement inclus dans un autre)
    raw.sort(key=lambda x: len(x[0]))  # du plus court (le plus spécifique) au plus long
    kept: list[tuple[str, str | None]] = []
    for txt, href in raw:
        if any(txt in other_txt for other_txt, _ in kept):
            continue
        kept.append((txt, href))
    return kept
...
def parse_listing_block(block: str, source_name: str, source_url: str,
    ...
    is_complete = adresse_precise and has_price
...
def enrich_listing(item: dict, timeout: int = 12) -> dict:
    """Va chercher la fiche complète de l'annonce (item['Lien']) pour compléter les charges
    et/ou l'adresse quand l'extrait de la page de recherche ne suffisait pas. Ne modifie que
    les champs encore manquants — n'écrase jamais une valeur déjà extraite avec succès."""
    if not item.get("Lien_direct"):
        return item  # pas de fiche individuelle distincte à aller chercher

    html = fetch(item["Lien"], timeout=timeout)
    if not html:
        return item
    soup = BeautifulSoup(html, "html.parser")
    full_text = soup.get_text(" ", strip=True)
    ...
    if updated.get("Charges_eur_m2_an") is None and updated.get("Loyer_HT_HC_eur_m2_an") is None:
        if ALL_INCLUSIVE_RE.search(full_text) and updated.get("Loyer_facial_eur_m2_an") is not None:
            ...
            cm = CHARGES_M2_RE.search(full_text)
            v = None
            if cm:
                v = _clean_int(cm.group(1))
            else:
                cmens = CHARGES_MENSUEL_RE.search(full_text)
                if cmens and surface:
                    raw = _clean_int(cmens.group(1))
                    if raw:
                        v = round(raw * 12 / surface, 1)
                else:
                    cann = CHARGES_ANNUEL_RE.search(full_text)
                    if cann and surface:
                        raw = _clean_int(cann.group(1))
                        if raw:
                            v = round(raw / surface, 1)
            if v is not None and 0 < v <= 150:
                updated["Charges_eur_m2_an"] = v
                if updated.get("Loyer_facial_eur_m2_an") is not None:
                    updated["Loyer_HT_HC_eur_m2_an"] = round(updated["Loyer_facial_eur_m2_an"] + v, 1)

    # adresse précise, si toujours inconnue
    if not updated.get("Adresse_precise"):
        ...
        if addr_m:
            ...
            updated["Complete"] = updated["Adresse_precise"] and (
                updated.get("Loyer_facial_eur_m2_an") is not None
                or updated.get("Prix_vente_eur_m2") is not None
            )
...
def enrich_listings(items: list[dict], max_items: int = 30, progress_cb=None) -> list[dict]:
    """Enrichit une liste d'annonces (typiquement déjà filtrée/réduite — voir max_items
    pour limiter le nombre de requêtes supplémentaires). progress_cb(i, n) optionnel."""
    out = []
    to_enrich = [it for it in items if it.get("Needs_detail_fetch") or not it.get("Adresse_precise")]
    ...
    for i, it in enumerate(to_enrich):
        ...
        time.sleep(0.1)
    for it in items:
        out.append(enrich_map.get(id(it), it))
    return out
...
def _run_search(url: str, source_name: str, commune: str, code_postal: str, type_bien: str) -> list[dict]:
    """Logique commune à toutes les sources : fetch, détection JS, découpage, extraction, dédup."""
    html = fetch(url)
    if html is None:
        return []
    if looks_like_js_shell(html):
        print(f"[live_search] {source_name} : page probablement rendue en JavaScript — "
              f"extraction impossible avec requests seul. ({url})")
        return []

    soup = BeautifulSoup(html, "html.parser")
    blocks = _split_listing_blocks(soup, url)
    out = []
    for b, href in blocks:
        item = parse_listing_block(b, source_name, href or url,
                                    fallback_commune=commune, fallback_cp=code_postal,
                                    fallback_type_bien=type_bien)
        if item:
            item["Lien_direct"] = href is not None
            if href is None:
                item["Observations"] = "⚠ Lien direct non trouvé, lien de recherche utilisé. " + item["Observations"]
            out.append(item)
...
def search_arthurloyd(commune: str, code_postal: str, operation: str = "location",
                       type_bien: str = "bureaux", region: str = "pays-de-la-loire") -> list[dict]:
    """URL confirmée par inspection directe du site (juin 2026) :
    https://www.arthur-loyd.com/bureau-location/pays-de-la-loire/nantes"""
    op = "location" if operation.lower().startswith("loc") else "vente"
    type_map = {
        "bureaux": f"bureau-{op}",
...
def search_tournymeyer(commune: str, code_postal: str, operation: str = "location",
                        type_bien: str = "bureaux", departement: str = "loire-atlantique") -> list[dict]:
    """Marque régionale JLL à Nantes. URL confirmée par inspection directe (juin 2026) :
    https://www.tournymeyer.fr/offres/location/bureaux/loire-atlantique/nantes/"""
...
def search_cbre(commune: str, code_postal: str, operation: str = "location",
                 ...
                 departement: str = "loire-atlantique") -> list[dict]:
    """⚠ NON VÉRIFIÉ — une seule URL CBRE a été observée (location-bureaux/Nantes) ;
    les variantes vente/activités/commerce ci-dessous sont une extrapolation du même
    schéma, pas une confirmation. À vérifier via diagnose() avant usage en confiance."""
    op = "location" if operation.lower().startswith("loc") else "vente"
    type_map = {"bureaux": "bureaux", "activités": "entrepots", "activites": "entrepots",
                "commerce": "commerces"}
    tb = type_map.get(type_bien.lower(), "bureaux")
...
def search_all_sources(commune: str, code_postal: str, operation: str = "location",
                        type_bien: str = "bureaux", sources: list[str] | None = None) -> list[dict]:
