...
""")
            if diag["brut"] == 0:
                ...
                # ── Encart debug : montre le texte brut réellement scrappé ──
                _debug_blocks = getattr(live_search, "LAST_DEBUG_BLOCKS", [])
                if _debug_blocks:
                    with st.expander("🔍 Debug — texte brut des 1res annonces (copie-le pour analyse)", expanded=True):
                        st.caption("Voici exactement ce que le scanner extrait. Le prix "
                                   "devrait apparaître ici — copie ce texte pour qu'on "
                                   "identifie le format exact à reconnaître.")
                        for i, blk in enumerate(_debug_blocks, 1):
                            st.text_area(f"Annonce {i}", blk, height=140, key=f"dbg_blk_{i}")
