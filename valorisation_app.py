...
def build_export_table(res_df: pd.DataFrame) -> pd.DataFrame:
    """Format nettoyé Savills :
    Date | Photo | État | Adresse complète | Surface (m²) | Loyer HT-HC €/m²/an | Commentaire | Lien"""
    rows = []
    for _, r in res_df.iterrows():
        ...
        # commentaire : utilise celui renseigné par l'utilisateur, sinon auto-généré
...
def export_to_excel_bytes(export_df: pd.DataFrame, title: str = "Extraction comparables") -> bytes:
    ...
    COL_WIDTHS = {"Date": 14, "Photo": 22, "État": 16,
                  "Adresse complète": 40, "Surface (m²)": 12,
                  "Prix €/m²": 16, "Commentaire": 44, "Lien": 36}
    ...
    for i, row_ in export_df.iterrows():
        ...
        for j, col in enumerate(COLS, start=1):
            ...
            c.alignment = LEFT if col in ("Adresse complète", "Commentaire", "Lien") else CENTER
            if col == "Lien" and val and str(val).startswith("http"):
                c.hyperlink = str(val)
                c.value = "🔗 Voir l'annonce"
                c.font = Font(name="Calibri", size=10, color="008493", underline="single")
                continue
            if bg: c.fill = bg
