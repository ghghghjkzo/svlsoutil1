...
def build_export_table(res_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in res_df.iterrows():
        ...
        # priorité : loyer HT/HC réellement calculé (facial + charges) ; à défaut, facial seul
        loyer_ht_hc = to_num(r.get("Loyer_HT_HC_eur_m2_an"))
        loyer_facial = to_num(r.get("Loyer_facial_eur_m2_an"))
        loyer_final = loyer_ht_hc if loyer_ht_hc is not None else loyer_facial
        loyer_note = "" if loyer_ht_hc is not None else (
            " (facial seul, charges non précisées)" if loyer_facial is not None else "")
        ...
        rows.append({
            ...
            "Loyer € H.T-H.C/m²/an": loyer_final if loyer_final is not None else "Non disponible",
...
if n_implausible:
    st.caption(f"⚠️ {n_implausible} valeur(s) hors fourchette plausible "
                f"({bounds[0]:.0f}–{bounds[1]:.0f}) écartée(s) du calcul (probable erreur de saisie/extraction).")

cible = (vals * weights).sum() / weights.sum()
basse, haute, med = vals.min(), vals.max(), vals.median()
unit = "€/m²" if is_vente_field else "€/m²/an"
...
show_cols = ["_score", "_dist", "Adresse", "Commune", "Type_actif", "Surface_m2",
             field, "Loyer_HT_HC_eur_m2_an", "Date_transaction", "Fiabilite", "Source", "Lien"]
