...
if mode == "Charger un fichier Excel":
    ...
    if run_live:
        ...
        with st.spinner(f"Recherche en direct sur {search_commune} ({search_cp})…"):
            collected = []
            for o in ops_to_search:
                for t in types_to_search:
                    collected += live_search.search_all_sources(search_commune, search_cp, o, t,
...
if len(vals) == 0:
    ...
    st.stop()
if n_implausible:
    st.caption(f"⚠️ {n_implausible} valeur(s) hors fourchette plausible "
                f"({bounds[0]:.0f}–{bounds[1]:.0f}) écartée(s) du calcul.")
...
c1.metric("Valeur basse" if is_vente_field else "Loyer bas", f"{basse:,.0f} {unit}".replace(",", " "))
c2.metric("Valeur cible" if is_vente_field else "Loyer cible", f"{cible:,.0f} {unit}".replace(",", " "))
c3.metric("Valeur haute" if is_vente_field else "Loyer haut", f"{haute:,.0f} {unit}".replace(",", " "))
c4.metric("Comparables retenus", len(res))

n = len(vals)
conf = "🟢 Élevée" if n >= 8 else ("🟡 Moyenne" if n >= 4 else "🔴 Faible")
st.caption(f"Médiane : {med:,.0f} {unit} — Fiabilité : {conf} ({n} comparable(s))".replace(",", " "))
if n < 4:
    st.error("Peu de comparables exploitables : élargis le rayon.")

# ---- tableau éditable : colonnes utiles uniquement, noms lisibles ----
st.subheader(f"Comparables ({len(res)}) — modifiables avant export")
st.caption("Tu peux corriger directement les valeurs dans le tableau — les modifications "
            "sont répercutées dans l'export Excel.")
...
avail = {k: v for k, v in COLS_KEEP.items() if k in res.columns}
display = res[list(avail.keys())].copy().rename(columns=avail)

# s'assure que Loyer HT-HC est toujours calculé même si la colonne était absente
if "Loyer HT-HC €/m²/an" not in display.columns:
    display["Loyer HT-HC €/m²/an"] = None

# data_editor = cellules modifiables
...
# recalcul dynamique du loyer cible depuis la table éditée
if not is_vente_field and "Loyer HT-HC €/m²/an" in edited.columns:
    new_vals = edited["Loyer HT-HC €/m²/an"].apply(to_num).dropna()
    new_vals = new_vals[new_vals.between(*bounds)]
    if len(new_vals) > 0:
        cible_edit = new_vals.mean()
        if abs(cible_edit - cible) > 0.5:
            st.info(f"💡 Après modifications : loyer cible recalculé → **{cible_edit:,.0f} €/m²/an HT-HC**".replace(",", " "))

# ---- export depuis la table éditée ----
st.divider()
st.subheader("Export pour rapport")

# reconstruire un df cohérent avec build_export_table depuis les données éditées
edited_for_export = edited.rename(columns={v: k for k, v in avail.items()})
