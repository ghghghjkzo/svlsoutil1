...
def make_map(target_lat: float, target_lon: float, target_label: str,
    ...
    m = folium.Map(location=center, zoom_start=14,
                   tiles=None)   # pas de fond par défaut, on les ajoute manuellement
...
tab_analyse, tab_carte = st.tabs(["📊 Analyse & comparables", "🛰️ Carte"])
...
with tab_carte:
    # ── Carte plein écran ─────────────────────────────────────────────────
    st.caption(
        "🔴 Actif cible · 🟢 Neuf · 🔵 Restructuré · 🟠 Seconde main · ⚫ État inconnu  "
        "| Clic sur un marqueur → détails de l'annonce | Bouton en haut à droite pour basculer satellite ↔ plan"
    )
    ...
    # hauteur maximale — prend tout l'espace de la page
    components.html(folium_map._repr_html_(), height=750, scrolling=False)
